...
class LoadData:
    ...
    def get_hold_num(self, hold_num=50, start_sorted=100, the_end_month=None, fixed_by_month=True):
        ...
        if the_end_month is None:
            ...
            if the_end_month not in df.index:  # Check if the_end_month exists in the index
                raise KeyError(f'The specified month "{the_end_month}" does not exist in the DataFrame.')
...
class BackTest:
    ...
    def fixed_pos2db(self, t_name=None, hold_sorted=0):
        ...
        cnt = 1  # 持仓第一天
        # target_ = self.df_stocks.loc[index_list[1]].code.tolist() #从股票数据中获取第一个交易日的选股结果， 总计200只
        ...
        for i in range(2, len(index_list)):
            ...
            if cnt < self.period:  # 非调仓日 判断是否为调仓日：
                ...
                # df_target_ = self.df_stocks.loc[index_list[i]].copy() #根据前一个交易日的股票推荐列表，获取今日的股票数据。
                ...
                hold_in_pre = list(set(pre_list) & set(hold_not_st_list))  # 按排名保留的票中剔除今日被ST的  好像不是被st，而是持仓重合的票
