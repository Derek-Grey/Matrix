...
class BackTest:
    ...
    def fixed_chg2db(self, t_name=None, hold_sorted=0, df_chg=None):
        ...
        turnover.append((index_list[1][1], 0))
        hold_positions.append((index_list[1][1], real_hold_position))
        ...
        for i in range(2, len(index_list)):
            ...
            if cnt < self.period:
                cnt += 1
                turnover.append((index_list[i][1], 0))
                hold_positions.append((index_list[i][1], real_hold_position))
            else:
                cnt = 1
                df_target_ = self.df_stocks.loc[index_list[i - 1]].copy()
                df_target_.set_index('code', inplace=True)
                ...
                turnover.append((index_list[i][1], len(list(set(real_hold_position) - set(real_target_position))) / self.hold_num))
                hold_positions.append((index_list[i][1], real_target_position))
                real_hold_position = real_target_position
        ...
        return df_stra, df_hold_positions

# %% [markdown]
# ## 3.实例化示例

# %%
ld = LoadData(date_s='2022-07-25', date_e='2022-09-01')
...
df_stk=pd.read_csv(r'C:\Users\Christopher\Desktop\shuidui\code\Daily\stra_V3_11.csv')
