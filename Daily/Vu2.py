...
def run_backtest_dynamic(stocks_matrix, limit_matrix, risk_warning_matrix, trade_status_matrix, score_matrix, 
                        hold_count=20, rebalance_frequency=1, start_sorted=100, the_end_month=None, fixed_by_month=True):
    """动态持仓策略回测
    
    Args:
        stocks_matrix: 股票收益率矩阵
        limit_matrix: 涨跌停矩阵
        risk_warning_matrix: 风险警告矩阵
        trade_status_matrix: 交易状态矩阵
        score_matrix: 评分矩阵
        hold_count: 基准持仓数量
        rebalance_frequency: 换仓周期
        start_sorted: 动态持仓起始排名
        the_end_month: 指定月份后固定持仓数量
        fixed_by_month: 是否在指定月份后固定持仓数量
    """
    start_time = time.time()
    
    # 计算每月股票数量
    monthly_counts = stocks_matrix.resample('M').apply(lambda x: x.notna().sum())
    ...
    logger.info(f"动态持仓策略回测完成，耗时: {time.time() - start_time:.2f} 秒")
...
def main():
    ...
    # 运行回测
    position_history = run_backtest_dynamic(aligned_stocks_matrix, aligned_limit_matrix, aligned_riskwarning_matrix, aligned_trade_status_matrix, score_matrix, hold_count, rebalance_frequency)
