...
def run_backtest(stocks_matrix, limit_matrix, risk_warning_matrix, trade_status_matrix, score_matrix, hold_count, rebalance_frequency):
    ...
    # 从第二天开始进行回测
    has_positions = False  # 标志位，指示是否已建立持仓
    rebalance_countdown = 0  # 换仓倒计时
    ...
    for day in range(1, len(stocks_matrix)):
        ...
        # 判断是否需要换仓
        if has_positions and rebalance_countdown == 0:
            ...
            rebalance_countdown = rebalance_frequency  # 重置换仓倒计时
        else:
            if not has_positions and len(previous_positions) > 0:
                has_positions = True  # 首次建立持仓
        ...
        if previous_date in stocks_matrix.index:
            ...
            daily_return = daily_returns.mean() if not daily_returns.empty else np.nan
            position_history.loc[position_history.index[day], "daily_return"] = daily_return

        # 更新换仓倒计时
        if has_positions:
            rebalance_countdown -= 1
