...
def auto_update_data(func):
    def wrapper(data_loader: LoadData, *args, **kwargs):
        local_files = [
            'data/aligned_stocks_matrix.csv',
            'data/aligned_limit_matrix.csv',
            'data/aligned_riskwarning_matrix.csv',
            'data/aligned_trade_status_matrix.csv',
            'data/aligned_score_matrix.csv'
        ]

        if not all(os.path.exists(f) for f in local_files):
            logger.debug('本地文件不存在，加载数据...')
            df_stocks, trading_days = data_loader.get_chg_wind()
            trade_status_matrix, riskwarning_matrix, limit_matrix = data_loader.get_stocks_info()[1:]
            ...
            save_matrices(data_loader, aligned_stocks_matrix, aligned_limit_matrix, 
                          aligned_riskwarning_matrix, aligned_trade_status_matrix, score_matrix)
        else:
            logger.debug('所有本地文件存在，加载数据...')
            aligned_stocks_matrix = pd.read_csv(local_files[0], index_col=0, parse_dates=True)
            aligned_limit_matrix = pd.read_csv(local_files[1], index_col=0, parse_dates=True)
            aligned_riskwarning_matrix = pd.read_csv(local_files[2], index_col=0, parse_dates=True)
            aligned_trade_status_matrix = pd.read_csv(local_files[3], index_col=0, parse_dates=True)
            score_matrix = pd.read_csv(local_files[4], index_col=0, parse_dates=True)

        return func(data_loader, aligned_stocks_matrix, aligned_limit_matrix, 
                    aligned_riskwarning_matrix, aligned_trade_status_matrix, score_matrix, *args, **kwargs)
    return wrapper

def save_matrices(data_loader, *matrices):
    file_names = [
        'aligned_stocks_matrix.csv',
        'aligned_limit_matrix.csv',
        'aligned_riskwarning_matrix.csv',
        'aligned_trade_status_matrix.csv',
        'aligned_score_matrix.csv'
    ]
    
    for matrix, file_name in zip(matrices, file_names):
        try:
            data_loader.save_matrix_to_csv(matrix, file_name)
        except Exception as e:
            logger.error(f"保存 {file_name} 失败: {e}")

@auto_update_data
def process_data(data_loader: LoadData, aligned_stocks_matrix, aligned_limit_matrix, 
                 aligned_riskwarning_matrix, aligned_trade_status_matrix, score_matrix):
    try:
        all_dates = pd.date_range(start=data_loader.date_s, end=data_loader.date_e)
        missing_dates = all_dates[~all_dates.isin(aligned_stocks_matrix.index)]

        if missing_dates.size > 0:
            logger.debug('发现缺失日期，加载数据...')
            df_stocks, trading_days = data_loader.get_chg_wind()
            trade_status_matrix, riskwarning_matrix, limit_matrix = data_loader.get_stocks_info()[1:]
            score_matrix = data_loader.generate_score_matrix('stra_V3_11.csv')
            ...
            for date in missing_dates:
                aligned_stocks_matrix.loc[date] = [np.nan] * aligned_stocks_matrix.shape[1]
                aligned_limit_matrix.loc[date] = [np.nan] * aligned_limit_matrix.shape[1]
                aligned_riskwarning_matrix.loc[date] = [np.nan] * aligned_riskwarning_matrix.shape[1]
                aligned_trade_status_matrix.loc[date] = [np.nan] * aligned_trade_status_matrix.shape[1]
...
def main():
    ...
    process_data(data_loader)  # 调用数据处理函数
